...
export const getSettlementData = query({
  ...
  handler: async (ctx, args) => {
    ...
    if (args.entityType === "user") {
      ...
      // Use a single signed balance: positive = they owe me, negative = I owe them
      let netBalance = 0;
      ...
      for (const st of settlements) {
        if (st.paidByUserId === me._id) {
          // I paid them ⇒ they owe me more (or I owe them less)
      ...
      const owing = netBalance < 0 ? -netBalance : 0;
      ...
      group.members.forEach((m) => {
        if (m.userId !== me._id) balances[m.userId] = 0; // Signed balance
      });
      ...
      for (const exp of expenses) {
        if (exp.paidByUserId === me._id) {
          ...
          const split = exp.splits.find((s) => s.userId === me._id && !s.paid);
          if (split) balances[exp.paidByUserId] -= split.amount;
        }
      }
